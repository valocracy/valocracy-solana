...
class AuthService {
						...
						<tr>
						  <td class="content-cell">
							<div class="f-fallback">
							  ...
							  <p>Use o código de seguranca a seguir para prosseguir com o login da conta <b>${data.email}</b> no <b>Valocracy</b>.</strong></p>
							  <!-- Action -->
	...
	async sendMailCode(data: any) {
		...
		const text = `Use o código de seguranca a seguir para prosseguir com o login da conta ${emailMasked} no Valocracy\n\n${data.code}\n\nAbraços,\nEquipe Valocracy`;

		await mailer.send({
			from: env.MAIL_SENDER,
			to: data.recipient_email,
			subject: 'Código de segurança de login da conta do Valocracy',
			text: text,
			html: emailHtmlBody
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		} as any);
	...
	async getNewJWT(loginData: LoginDataInterface) {
		const { user_id, username, user_discord } = loginData;
		const jwtSecret = env.JWT_SECRET;
		const refreshId = user_id + jwtSecret;
		...
		const expiration = moment().add(7, 'day');
		// const expirtaionUnix = expiration.unix();

		const token = jwt.sign(jwtSignData, jwtSecret, {
			expiresIn: '7d',
		});
